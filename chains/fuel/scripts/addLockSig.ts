...
async function addLockSig() {
  ...
  console.log(
    'signiature verifeid off chain: ',
    signerWallet.address.toB256() == Signer.recoverAddress(sha256(msgBytes), signature).toB256()
  );
  ...
  try {
    ...
    console.log('add_lock_sig function logs: ', logs[0]);
    console.log('add_lock_sig function result:', value);
  } catch (error) {
    console.error('Error calling add_lock_sig function:', error);
  }
}
