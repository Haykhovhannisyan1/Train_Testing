...
contract Train is NilBase {
  ...
  ) public onlyInternal returns (bytes32) {
    HTLC storage htlc = contracts[Id];
  ...
  function redeem(bytes32 Id, uint256 secret) public _exists(Id) onlyInternal returns (bool) {
    ...
    Nil.asyncCall(htlc.srcReceiver, msg.sender, address(this), 0, Nil.FORWARD_REMAINING, htlc.amount, '');
